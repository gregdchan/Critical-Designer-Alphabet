...
export const BADGES: Badge[] = [
  ...
  },
  {
    ...
    color: '#3b82f6',
    requirement: (participant, responses) =>
      responses.filter(r => r.votes?.some((v: any) => v.participantId === participant.id)).length >= 10,
    points: 20
  },
  {
    id: 'lens-explorer',
  ...
  },
  {
    ...
    requirement: (participant, responses) => {
      ...
      const userResponses = responses.filter(r => r.participantId === participant.id);
      const votesGiven = responses.filter(r => r.votes?.some((v: any) => v.participantId === participant.id)).length;
...
export function calculateParticipantScore(participant: any, responses: any[], timeline: any[]): number {
  let totalScore = 0;
