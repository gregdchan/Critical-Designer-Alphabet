...
function hasLongText(values: string[]) {
	return values.some(
		(v) => (v?.trim()?.length ?? 0) >= 30 || (/\s/.test(v) && v.trim().length >= 10)
	);
}
...
export function inferQuestionType(question: Question, responses: Response[]): InferredQuestionType {
	...
	if (explicit) {
		...
		if (['multiple_choice', 'multiselect', 'singlechoice', 'singlechoice', 'choice'].includes(norm))
	...
	const hasVotes =
		(responses ?? []).some((r) => (r?.votes ?? 0) > 0) || Boolean(question?.enable_voting);
		...
		case 'multipleChoicePie': {
			...
			return {
				...
				data: {
					...
					meta: {
						kind: 'multipleChoice',
						viz: type === 'multipleChoicePie' ? 'pie' : 'bar',
