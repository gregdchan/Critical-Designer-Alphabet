  ...
  function render(root: SVGGElement, innerWidth: number, innerHeight: number) {
    ...
    data.forEach((phaseData, idx) => {
      ...
      nodes
        ...
        .attr('opacity', (d: any) =>
          highlightParticipantId ? (d.data.participantId === highlightParticipantId ? 0.95 : 0.2) : 0.85
        )
        .attr('stroke', (d: any) =>
          highlightParticipantId && d.data.participantId === highlightParticipantId ? theme.brand : 'hsl(var(--surface-elevated))'
        )
      ...
      nodes
        ...
        .text((d: any) => {
          ...
          return (d.data.label || '').slice(0, maxChars) + ((d.data.label || '').length > maxChars ? '…' : '');
        });
    });
  }
</script>
...
<ChartFrame {title} ariaLabel="Top ideas by phase" let:innerWidth let:innerHeight>
  ...
  <div slot="tooltip" bind:this={tooltipEl} style="position:fixed;opacity:0;pointer-events:none;background:hsl(var(--surface-elevated));border:1px solid hsl(var(--brand));border-radius:8px;padding:8px 10px;font-size:12px;color:hsl(var(--text-primary));box-shadow:0 6px 18px hsl(var(--brand) / 0.15);max-width:300px" />
</ChartFrame>
