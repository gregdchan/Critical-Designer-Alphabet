<script lang="ts">
  ...
  const lensColor = (lens: string | undefined, fallback: string) => {
    ...
    const entry = Object.entries(lensPalette).find(([key]) =>
      key.toLowerCase() === lens.toLowerCase()
    );
  ...
  function renderChart() {
    ...
    if (hasData) {
      ...
      grid.selectAll('line').attr('stroke', 'rgba(148, 163, 184, 0.18)').attr('stroke-dasharray', '4 10');
    }
    ...
    nodeGroup
      .on('pointerenter', function (event, d) {
        ...
        tooltip
          ...
          .style('transform', `translate(${event.clientX - bounds.left + 16}px, ${event.clientY - bounds.top - 24}px)`);
      })
    ...
    const legendItems = legend
      ...
      .data(
        Array.from(
          d3.group(nodes, (node) => node.lens).keys()
        )
      )
      .enter()
      .append('g')
