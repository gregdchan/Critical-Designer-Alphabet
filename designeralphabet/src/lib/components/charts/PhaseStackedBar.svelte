<script lang="ts">
  ...
  $: lenses = Array.from(
    new Set(data.flatMap((d) => d.stacks.map((s) => s.lens)))
  );
  ...
  function render(root: SVGGElement, innerWidth: number, innerHeight: number) {
    ...
    const x = scaleLinear().domain([0, maxTotal]).range([0, innerWidth - (isMobile ? 0 : 80)]);
    ...
    legend
      ...
      .each(function (lens, i) {
        ...
        it.append('rect').attr('width', 10).attr('height', 10).attr('fill', colorFor(lens, i)).attr('rx', 2);
        it
          .append('text')
          .attr('x', 14)
          .attr('y', 9)
...
<ChartFrame {title} ariaLabel="Phase stacked bar" let:innerWidth let:innerHeight>
  ...
  <div slot="tooltip" bind:this={tooltipEl} style="position:fixed;opacity:0;pointer-events:none;background:hsl(var(--surface-elevated));border:1px solid hsl(var(--brand));border-radius:8px;padding:8px 10px;font-size:12px;color:hsl(var(--text-primary));box-shadow:0 6px 18px hsl(var(--brand) / 0.15)" />
</ChartFrame>
