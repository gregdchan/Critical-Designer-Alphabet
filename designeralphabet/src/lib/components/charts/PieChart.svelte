<script lang="ts">
	...
	function render(
		...
		const pieGen = pie<any>()
			.sort(null)
			.value((d: any) => Math.max(0, d.value));
	...
	$: percentage =
		selectedSlice && data
			? (
					(selectedSlice.value / (data.series[0]?.points.reduce((s, p) => s + p.value, 0) || 1)) *
					100
				).toFixed(1)
			: '0';
</script>
...
<div class="w-full h-full flex flex-col gap-4">
	<ChartFrame {title} {ariaLabel} let:innerWidth let:innerHeight>
		...
		<div
			slot="tooltip"
			bind:this={tooltipEl}
			style="position:fixed;opacity:0;pointer-events:none;background:hsl(var(--surface-elevated));border:1px solid hsl(var(--brand));border-radius:8px;padding:8px 10px;font-size:12px;color:hsl(var(--text-primary));white-space:nowrap;box-shadow:0 6px 18px hsl(var(--brand) / 0.15)"
		/>
	</ChartFrame>

	{#if selectedSlice}
		<div
			class="rounded-xl border-2 border-purple-400/30 bg-purple-50/80 p-4 shadow-lg backdrop-blur-sm transition-all"
		>
	...
	{:else}
		<div class="rounded-xl border-2 border-slate-200 bg-slate-50 p-4 text-center">
			<p class="text-slate-500 text-sm">
				Click or tap on a slice to see the full label and details
			</p>
		</div>
	{/if}
</div>
