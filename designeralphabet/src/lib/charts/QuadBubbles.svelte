<script lang="ts">
	...
	function updateVisualization() {
		...
		simulation = forceSimulation(bubbleData)
			...
			.force('collision', forceManyBody().strength(1).distanceMax(d => d.radius + 10))
			.force('x', forceX().x(d => (lensPositions[d.lens]?.x || 0.5) * innerWidth).strength(0.3))
			.force('y', forceY().y(d => (lensPositions[d.lens]?.y || 0.5) * innerHeight).strength(0.3))
			.alpha(0.8)
			.alphaDecay(0.02);
		...
		// Bind data
		bubbles = chart.selectAll('.bubble-group')
