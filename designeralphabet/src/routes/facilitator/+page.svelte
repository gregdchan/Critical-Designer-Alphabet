                ...
                <div class="space-y-3 text-xs">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <span class="text-white ml-2">{template.sections?.breakout?.rounds?.length || 0}</span>
                    </div>
                    <div>
                      <span class="text-slate-400">Total Duration:</span>
                      <span class="text-white ml-2">
                        {(template.sections?.breakout?.rounds ?? []).reduce((total, round) => total + (round.minutes || 0), 0)}min
                      </span>
                    </div>
                  </div>
                  ...
                  <div class="flex flex-wrap gap-2">
                    {#if template.aiAssist?.enabled}
                      <span class="px-2 py-1 bg-purple-400/20 text-purple-300 rounded flex items-center gap-1">
                        🤖 AI Assist
                      </span>
                    {/if}
                ...
                <div>
                  ...
                  <div class="flex flex-wrap gap-1 mt-1">
                    {#each selectedTemplate.facilitation?.roles ?? [] as role}
                      <span class="px-2 py-1 bg-cyan-400/20 text-cyan-300 rounded text-xs">{role}</span>
                    {/each}
                  </div>
                </div>
                {#if selectedTemplate.facilitation?.scoring}
                  <div>
                    ...
                    <div class="text-xs text-slate-300 mt-1">
                      Idea: {selectedTemplate.facilitation.scoring.idea}pts,
                      Vote: {selectedTemplate.facilitation.scoring.vote}pts,
                      Reflection: {selectedTemplate.facilitation.scoring.reflection}pts
                    </div>
                  </div>
                {/if}
                  ...
                  <div>
                    ...
                    <div class="flex flex-wrap gap-1 mt-1">
                      {#each selectedTemplate.visuals.charts as chart}
                        <span class="px-2 py-1 bg-blue-400/20 text-blue-300 rounded text-xs">{chart}</span>
                      {/each}
                    </div>
                  </div>
                {/if}
                ...
                {#if selectedTemplate.resources && selectedTemplate.resources.length > 0}
                  {#each selectedTemplate.resources.slice(0, 3) as resource}
                    <div class="text-slate-300">
                      <a href={resource.url} target="_blank" class="hover:text-yellow-300 transition-colors">
                        📎 {resource.title}
                      </a>
                    </div>
                  {/each}
              ...
              {#each selectedTemplate.sections.breakout.rounds as round, index}
                <div class="bg-slate-700/50 rounded-lg p-4">
                  <div class="flex items-center gap-3 mb-3">
                    <span class="w-8 h-8 bg-gradient-to-r from-cyan-400 to-purple-400 text-slate-900 rounded-full flex items-center justify-center text-sm font-bold">
                      {index + 1}
                    </span>
                    <div>
                  ...
                  {#if round.questions && round.questions.length > 0}
                    <div>
                      <span class="text-sm text-slate-400 mb-2 block">Questions ({round.questions.length}):</span>
                ...
                <div>
                  ...
                  <div class="flex flex-wrap gap-2">
                    {#each selectedTemplate.sections.synthesis.methods as method}
                      <span class="px-3 py-2 bg-green-400/20 text-green-300 rounded text-sm">{method}</span>
                    {/each}
                  </div>
                </div>
              {/if}
