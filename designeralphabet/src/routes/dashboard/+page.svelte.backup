	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
		<div class="space-y-8">
			...
			<div class="flex items-center justify-between">
				<div>
					<h1 class="text-3xl font-bold text-slate-900">Planning Dashboard</h1>
					<p class="text-slate-600 mt-1">Comprehensive workshop analytics and session management</p>
				</div>
				<button
			...
			{#if error}
				<div class="flex items-center justify-center py-12">
					<div class="text-center bg-white rounded-xl border border-slate-200 p-8 shadow-sm">
						<div class="text-red-600 mb-2 font-semibold">⚠️ Error loading dashboard</div>
						<div class="text-slate-600 text-sm">{error}</div>
						<button
			...
			{:else if loading}
				<div class="flex items-center justify-center py-12">
					<div class="text-slate-600">Loading dashboard data...</div>
				</div>
			{:else}
				<!-- Key Metrics -->
				<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
					<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-2xl font-semibold text-slate-900">{dashboardMetrics.totalSessions}</p>
							</div>
						</div>
					</div>
					...
					<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="text-sm text-slate-600">Ideas Generated</p>
								<p class="text-2xl font-semibold text-slate-900">{dashboardMetrics.totalIdeas}</p>
							</div>
						</div>
					</div>
					...
					<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="text-sm text-slate-600">Avg Engagement</p>
								<p class="text-2xl font-semibold text-slate-900">{dashboardMetrics.avgEngagement}</p>
							</div>
						</div>
					</div>
					...
					<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-2xl font-semibold text-slate-900">{dashboardMetrics.activeSessions}</p>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="grid gap-8 lg:grid-cols-3">
					...
					<div class="lg:col-span-1">
						<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
							<h2 class="text-lg font-semibold text-slate-900 mb-6">Recent Sessions</h2>
							<div class="space-y-4">
								{#each sessions as session}
									...
									>
										<div class="flex items-center justify-between mb-2">
											<h3 class="font-medium text-slate-900">{session.name}</h3>
											<span
										...
										<div class="text-sm text-slate-600 space-y-1">
											<p>Code: <span class="font-mono text-slate-900">{session.code}</span></p>
					...
					<div class="lg:col-span-2">
						{#if loadingSession}
							<div class="bg-white rounded-xl border border-slate-200 p-12 text-center shadow-sm">
								<div class="text-slate-600">Loading session details...</div>
							</div>
							...
							<div class="space-y-6">
								...
								<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
									<div class="flex items-center justify-between mb-4">
										<div>
											<h2 class="text-xl font-semibold text-slate-900">{selectedSession.name}</h2>
											<p class="text-slate-600">Template: {selectedSession.template}</p>
										</div>
										<div class="flex items-center gap-2">
											<button
												class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors shadow-sm"
									...
									<div class="grid grid-cols-4 gap-4">
										<div class="text-center">
											<p class="text-2xl font-semibold text-slate-900">{participants.length}</p>
											<p class="text-sm text-slate-600">Participants</p>
										</div>
										<div class="text-center">
											<p class="text-2xl font-semibold text-slate-900">{responses.length}</p>
											<p class="text-sm text-slate-600">Ideas</p>
										</div>
										<div class="text-center">
											<p class="text-2xl font-semibold text-slate-900">
												{responses.reduce((sum, r) => sum + (r.votes?.length || 0), 0)}
											</p>
											<p class="text-sm text-slate-600">Total Votes</p>
										</div>
										<div class="text-center">
											<p class="text-2xl font-semibold text-slate-900">
												{Math.round(
											...
											<p class="text-sm text-slate-600">Avg Votes/Idea</p>
										</div>
									</div>
								</div>
								...
								<div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
									...
									<div class="space-y-3">
										{#each leaderboard.slice(0, 5) as participant, index}
											<div class="flex items-center gap-4 p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors">
												<div class="flex items-center gap-3">
													<span class="text-lg font-semibold text-slate-700 w-6 text-center"
														>#{index + 1}</span
													>
													<div
													...
													<div>
														<p class="text-sm font-medium text-slate-900">{participant.name}</p>
