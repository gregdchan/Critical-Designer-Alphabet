<script lang="ts">
	...
	$: {
		...
		scaleMinLabel = stringOr((currentQuestionConfig as { minLabel?: unknown }).minLabel, 'Min');
		scaleMaxLabel = stringOr((currentQuestionConfig as { maxLabel?: unknown }).maxLabel, 'Max');
		landscapeXLabel = stringOr((currentQuestionConfig as { xLabel?: unknown }).xLabel, 'X Axis');
		landscapeYLabel = stringOr((currentQuestionConfig as { yLabel?: unknown }).yLabel, 'Y Axis');
	...
	async function submitResponse() {
		...
		if (!currentParticipant || !currentParticipant.id) {
			...
			alert(
				'Error: You must join the session with a name before submitting responses. Please refresh and enter your name.'
			);
		...
		>
			...
			<div class={isMobile ? 'w-full' : 'flex-1 overflow-auto'}>
						...
						<div class="flex flex-col gap-2">
							<div class="flex items-start justify-between gap-4">
								<div class="flex-1 min-w-0">
									...
									<h1
										class="text-lg md:text-2xl font-semibold text-primary flex flex-wrap items-center gap-2"
									>
									...
									{#if sessionInfo.challenge && !isMobile}
										<div class="mt-2 text-sm text-secondary max-w-2xl line-clamp-3">
											<PortableText
												value={sessionInfo.challenge}
												styleClass="prose-sm prose-slate"
											/>
										</div>
									{/if}
									<p class="mt-1 text-xs md:text-sm text-secondary truncate">
										{currentParticipant?.name ?? 'Anonymous'} · {currentParticipant?.role ??
											activeRole}
									</p>
								</div>
							</div>
				...
				<main class="mx-auto max-w-full space-y-6 md:space-y-8 px-4 md:px-6 py-6 md:py-8">
					...
					{#if isSessionEnded}
						<div
							class="rounded-xl border border-rose-700 bg-rose-600 p-4 shadow-lg shadow-rose-600/25"
						>
							<div class="flex items-center gap-3">
								...
								<div>
									...
									<p class="text-xs text-white/90">
										This session has concluded. You can review the final results below.
									</p>
								</div>
							</div>
						</div>
					{:else if isSessionPlanned}
						<div
							class="rounded-xl border border-amber-700 bg-amber-500 p-4 shadow-lg shadow-amber-500/25"
						>
							<div class="flex items-center gap-3">
								...
								<div>
									...
									<p class="text-xs text-white/90">
										This session is scheduled but hasn't started yet. Please wait for the
					...
					{:else if isSessionLive}
						<div
							class="rounded-xl border border-green-700 bg-green-600 p-4 shadow-lg shadow-green-600/25"
						>
							<div class="flex items-center gap-3">
								...
								<div>
									...
									<p class="text-xs text-white/90">
										This session is live. Your responses will be visible in real-time.
									</p>
								</div>
							</div>
						</div>
							...
							<div class="flex items-center justify-between mb-4">
								...
								{#if phaseCountdownLabel}
									<div class="text-right">
										...
										<p class="text-2xl md:text-3xl font-mono text-cyan-100">
											{phaseCountdownLabel}
										</p>
									</div>
								{/if}
							...
							{#if questionsList.filter((q) => q.phase_key === activePhase.phase_key || (!q.phase_key && activePhase.status === 'active')).length > 0}
								{@const phaseQuestions = questionsList.filter(
									(q) =>
										...
										(!q.phase_key && activePhase.status === 'active')
								)}
								<div class="mt-6">
									<div class="flex items-center justify-between mb-4">
											...
											<div class="rounded-lg border border-line panel p-4">
												...
												>
													<div class="flex-1 min-w-0">
														...
														<h5 class="text-sm font-semibold text-primary mt-1 break-words">
															{question.text}
														</h5>
													</div>
													<button
												...
												{#if responsesList.filter((r) => r.question_id === question.id).length > 0}
													...
													<div class="space-y-2 max-h-60 overflow-y-auto">
														{#each questionResponses as response}
															...
															{@const isOwnResponse =
																response.participant_id === currentParticipant?.id}
															...
															<div class="rounded border border-line panel p-3 text-xs">
																<div class="flex items-center justify-between text-secondary mb-1">
																	<div class="flex items-center gap-2">
																		...
																		{#if isOwnResponse}
																			<span
																				class="px-1.5 py-0.5 text-[10px] rounded bg-purple-500/20 text-purple-300"
																				>You</span
																			>
																		{/if}
																	</div>
												...
												{:else}
													<p class="text-xs text-secondary italic mt-2">
							...
							{:else}
								<div class="mt-6 rounded-lg border border-line surface-muted p-6 text-center">
									<p class="text-sm text-secondary">
										No questions available for this phase yet. The facilitator will add questions
										soon.
									</p>
								</div>
							{/if}
					...
					{:else if !isFacilitator()}
						...
						<section class="rounded-2xl border border-line panel p-8 text-center">
							...
							<p class="text-sm text-secondary">
								The facilitator hasn't started a phase yet. Please wait while they set up the
								session.
							</p>
						</section>
					{/if}
										...
										>
											...
											{#if questionsList.filter((q) => q.phase_key === phase.phase_key || (phase.status === 'active' && !q.phase_key)).length > 0}
												...
												<div class="mt-6">
													...
													<div class="space-y-4">
														{#each phaseQuestions as question}
															<div class="rounded-lg border border-line panel p-4 space-y-3">
																<div class="flex items-start justify-between gap-3">
																	<div class="flex-1">
																		<div class="flex items-center gap-2 mb-2">
																			<span
																				class="text-xs uppercase tracking-[0.3em] text-cyan-200"
																			>
																				{question.section}
																			</span>
																...
																{#if responsesList.filter((r) => r.question_id === question.id).length > 0}
																	...
																	<div class="space-y-2 max-h-40 overflow-y-auto">
																		{#each sortedByVotes as response}
																			<div class="rounded border border-line panel p-2 text-xs">
																				<div
																				...
																				>
																					...
																					{:else}
																						<span
																							class="inline-flex items-center gap-1 text-secondary text-xs"
																						>
																							<IconThumbUp class="h-3 w-3" />
																							{response.votes ?? 0}
																						</span>
					...
					<section class="grid gap-6 md:grid-cols-3">
						...
						{#if sessionInfo.active_round}
							<div
								class="md:col-span-3 rounded-xl border border-retro-purple/30 bg-purple-500/10 p-4"
							>
					...
					{#if isFacilitator()}
						<section class="rounded-2xl border border-line panel p-4 md:p-6">
							<nav
								class="flex gap-2 md:gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent"
							>
									...
									<div class="space-y-6">
										...
										<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
											<div class="bg-surface-muted rounded-xl border border-brand/20 p-4 h-80">
												<h3 class="text-sm font-semibold text-cyan-200 mb-3">
													Participation Pulse
												</h3>
												...
												<div class="space-y-2 max-h-64 overflow-y-auto">
													{#if leaderboardList.length > 0}
														{#each leaderboardList.slice(0, 8) as participant, index}
															<div
																class="flex items-center justify-between py-2 border-b border-line/50 last:border-0"
															>
																<div class="flex items-center gap-2">
													...
													{#each chatList as entry}
														<div class="rounded-lg border border-line panel p-3">
															<div class="flex items-center justify-between text-xs text-secondary">
																<span
																	>{participantsList.find((p) => p.id === entry.participant_id)
																		?.name ?? 'Anonymous'}</span
																>
										...
										{:else if activeTab === 'participants'}
											<div class="space-y-4">
												...
												<div class="grid gap-4 lg:grid-cols-4">
													...
													<div class="rounded-lg border border-brand/30 bg-brand/10 p-3">
														<p class="text-xs uppercase tracking-wide text-cyan-200">
															Total Responses
														</p>
														<p class="text-xl font-semibold text-primary">
													...
													<div
														class="rounded-lg border border-retro-purple/30 bg-retro-purple/10 p-3"
													>
														<p class="text-xs uppercase tracking-wide text-purple-200">
															Total Votes
														</p>
														<p class="text-xl font-semibold text-primary">
													...
													<div class="rounded-lg border border-yellow-400/30 bg-yellow-400/10 p-3">
														<p class="text-xs uppercase tracking-wide text-yellow-200">
															Chat Messages
														</p>
														<p class="text-xl font-semibold text-primary">
															...
															<div class="flex items-center justify-between">
																<div class="flex items-center gap-3">
																	...
																	<div>
																		...
																		<div
																			class="flex items-center gap-4 text-xs text-secondary mt-1"
																		>
																			...
																			{:else}
																				<span
																					>{Math.floor(participant.minutesSinceActivity / 1440)}d
																					ago</span
																				>
																			{/if}
																		</div>
									...
									{#each leaderboardList as player, index}
										...
										>
											...
											<span class="font-mono text-cyan-200"
												>{player.score ?? player.points ?? 0} pts</span
											>
										</li>
									{/each}
								</ul>
		...
		<div class="w-full max-w-xl rounded-2xl border border-line bg-surface-elevated p-6 shadow-2xl">
			...
			<div class="mt-4 space-y-4">
					...
					<label class="flex flex-col gap-2 text-sm text-secondary">
						...
						<div class="space-y-2">
							{#each modalOptions as option}
								<label
									class="flex items-center gap-2 p-2 rounded border border-line bg-surface-muted hover:border-brand/50 cursor-pointer"
								>
									<input
										type="radio"
				...
				{:else if modalResponseType === 'maturityDial'}
					...
					<MaturityDialInput question={currentQuestion} on:submit={handleAdvancedResponseSubmit} />
