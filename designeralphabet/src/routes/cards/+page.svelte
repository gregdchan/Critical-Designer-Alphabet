<script lang="ts">
  ...
  import { IconRefresh as Shuffle, IconStar as Sparkles, IconSearch as Search, IconX as X, IconArrowRight as ArrowRight } from '@tabler/icons-svelte';
...
<section class="space-y-12 text-white" aria-live="polite">
  <header class="space-y-6 rounded-[2.5rem] border border-white/10 bg-midnight/80 p-8 shadow-[0_0_35px_rgba(0,255,247,0.12)] sm:p-10">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div class="space-y-3">
        <p class="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-2 text-[0.65rem] uppercase tracking-[0.3em] text-white/70">
        ...
        <p class="max-w-3xl text-sm text-white/70">
          Filter by lens, remix prompts, and stage up to five cards. Every selection syncs to live sessions so your team can
          explore justice-centered design moves together.
        </p>
      </div>
    ...
    <div class="grid gap-4 sm:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
      <label class="group relative flex items-center gap-3 rounded-2xl border border-white/15 bg-black/50 px-4 py-3 shadow-[0_0_25px_rgba(255,255,255,0.04)]">
        ...
        />
        <div class="pointer-events-none absolute inset-0 rounded-2xl opacity-0 transition group-focus-within:opacity-100">
  ...
  {#if loading}
    <div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
      {#each Array.from({ length: 6 }) as _, index}
        <div class="animate-pulse rounded-[2rem] border border-white/10 bg-black/40 p-6 shadow-[0_0_30px_rgba(255,255,255,0.04)]">
  ...
  {:else if error}
    <div class="rounded-[2rem] border border-rose-500/50 bg-rose-500/10 p-6 text-sm text-rose-200">
      ...
      <p class="mt-2 text-xs text-rose-100/80">
        Verify your <code class="code">VITE_SANITY_*</code> variables and confirm the dataset is publicly readable.
      </p>
    </div>
  {:else if !cards.length}
    <div class="rounded-[2rem] border border-white/15 bg-black/50 p-6 text-sm text-white/70">{statusMessage}</div>
      ...
      {#each filteredCards as card, index (card._id)}
        <div>
          ...
          >
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1 space-y-2">
                <div class="flex items-center gap-3">
                  <span class="font-techno text-sm uppercase tracking-[0.2em] text-white">{card.title}</span>
                  {#if card.category}
                    <span class={`rounded-full border px-3 py-1 text-[0.6rem] uppercase tracking-[0.3em] ${variantClass(card, 'chip')}`}>
                      {card.category}
                    </span>
                  {/if}
                </div>
            ...
            {/if}
            <div class="flex flex-wrap gap-2 text-[0.6rem] uppercase tracking-[0.25em] text-white/60">
              {#if card.tags?.length}
                ...
                {#if card.tags.length > 4}
                  <span class="rounded-full border border-white/15 bg-white/5 px-3 py-1">+{card.tags.length - 4}</span>
                {/if}
              {/if}
            </div>
...
{#if selectedCards.length}
  <div class="pointer-events-none fixed inset-x-0 bottom-5 z-40 flex justify-center px-4">
    <div class="pointer-events-auto flex w-full max-w-4xl flex-col gap-4 rounded-[2rem] border border-neonCyan/40 bg-midnight/90 p-5 text-white shadow-[0_0_45px_rgba(0,255,247,0.2)] backdrop-blur">
  ...
  {#if activeCard}
    <article class="flex h-full flex-col gap-6 overflow-y-auto bg-midnight/95 p-6 text-white">
      <header class="flex items-start justify-between gap-4">
        <div class="space-y-3">
          <div class="flex items-center gap-3 text-[0.65rem] uppercase tracking-[0.3em] text-white/60">
          ...
          <p class="text-xs uppercase tracking-[0.35em] text-white/60">Letter {activeCard.letter}</p>
        </div>
        <button
      ...
      {#if activeCard.description}
        <section class="space-y-3 rounded-2xl border border-white/10 bg-black/40 p-4 text-sm text-white/75">
          <h3 class="text-xs uppercase tracking-[0.3em] text-neonCyan">Description</h3>
          <p>{activeCard.description}</p>
        </section>
      {/if}
      {#if activeCard.prompt}
        <section class="rounded-2xl border border-neonPink/40 bg-neonPink/10 p-4 text-sm text-white/85 shadow-neon-pink">
        ...
        <section class="space-y-3 text-sm text-white/75">
          ...
          <ul class="space-y-2">
            {#each activeCard.readingList as item}
              <li>
                {#if item.url}
                  <a class="text-neonCyan underline-offset-4 hover:underline" href={item.url} target="_blank" rel="noreferrer">
                    {item.title}
                  </a>
                {:else}
                  {item.title}
        ...
        <section class="space-y-3 text-sm text-white/75">
          ...
          <ul class="space-y-2">
            {#each activeCard.sources as source}
              <li>
                <a class="text-neonPink underline-offset-4 hover:underline" href={source} target="_blank" rel="noreferrer">{source}</a>
              </li>
            {/each}
          </ul>
        </section>
      ...
      <footer class="mt-auto space-y-3 text-xs text-white/60">
        ...
        <p>
          The Designer’s Critical Alphabet is by Dr. Lesley-Ann Noel. Respect licensing and context when sharing.
        </p>
      </footer>
    </article>
  {/if}
