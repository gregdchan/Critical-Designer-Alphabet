<script lang="ts">
  ...
  const heroTitle = page?.heroTitle ?? page?.title ?? 'About the Critical Designer Alphabet Platform';
  const heroSubtitle =
    page?.heroSubtitle ??
  ...
  const pageSections: SectionShape[] | undefined = page?.sections?.map((section, index) => ({
    ...
    accent: (section.accentColor as keyof typeof accentStyles | undefined) ?? fallbackSections[index]?.accent,
    icon: fallbackSections[index]?.icon ?? icons[index % icons.length]
  }));

  const sections = (pageSections?.length ? pageSections : fallbackSections).map((section, index) => {
...
<div class="space-y-12">
  ...
  {#each sections as section, index}
    <section class={`bg-slate-800 rounded-lg border border-slate-600 p-8 ${section.styles?.border ?? ''}`}>
      <div class="flex items-start gap-4 mb-6">
        <div class={`flex h-12 w-12 items-center justify-center rounded-lg ${section.styles?.header ?? 'bg-blue-600/20 text-blue-300'}`}>
